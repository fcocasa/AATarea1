...
DEFAULT_ENV = 'MiniGrid-SimpleCrossingS9N1-v0'  # 'MiniGrid-FourRooms-v0'
...
print(
    f"I start in {environment.agent_pos} looking {AGENT_DIR_TO_STR[environment.agent_dir]}")
for step, action in enumerate(SOME_ACTIONS):
    print(f"\n\tStep {step} - Action {action.name}")
    n_obs, reward, done, _ = environment.step(action)
    print(
        f"\tNow I'm in {environment.agent_pos} looking {AGENT_DIR_TO_STR[environment.agent_dir]}")
    if done:
        break
print(
...
last_cell_obs = n_obs['image'][environment.agent_pos[1],
                               environment.agent_pos[0]]
last_object = IDX_TO_OBJECT[last_cell_obs[2]
                            ] if last_cell_obs[2] > -1 else 'nothing'
